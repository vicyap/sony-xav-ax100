...
//DENNIS add for 8388
static void
draw_black(CoreWindowStack *stack, CardState *state, const DFBRegion *region )
{
     DFBRegion dest;
     ...
     if (!dfb_region_intersect( &dest, 0, 0, state->destination->config.size.w - 1, state->destination->config.size.h - 1 ))
         return;
     ...
     switch (stack->bg.mode) {
          case DLBM_COLOR: {
               ...
               if (DFB_PIXELFORMAT_IS_INDEXED( dst->config.format ))
                    dfb_state_set_color_index( state,  /* FIXME: don't search every time */
                                               dfb_palette_search( dst->palette, color->r, color->g, color->b, color->a ) );
               else
                    dfb_state_set_color( state, color );

               state->destination = state->tmpSurface;
               int tmpNum = state->to;

               //printf("[DirectFB] clean front!!\n");
               state->to = 0;
               dfb_gfxcard_fillrectangles( &rect, 1, state );
               ...
               int              y2 )
{
...
#ifdef window_clear
          //DENNIS modify for 8388
          if(window->usedCount==0)
          {
               //printf("[DirectFB] first use this window (window)\n");
               window->usedCount++;
               state->usedCount = 1;
               state->tmpSurface = window->surface;
          }
#endif
     ...
     {
#ifdef window_clear
          if(state->usedCount == 1)
          {
               //printf("[DirectFB] first use this window (background)\n");
               draw_black( stack, state, &region );
               state->usedCount = 0;
          }
#endif
...
#ifdef BACK_FRONT_CHANGE
  if(dfb_config->qt_flip && dfb_config->qt_flip_in==2)
  {
//       DFBRegion copy_updates = { 0, 0, dfb_config->copy_width-1, dfb_config->copy_height-1};     
       dfb_gfx_copy_regions( region->surface, CSBR_FRONT, region->surface, CSBR_BACK, &dfb_config->copy_region, 1, 0, 0 );
  }
#endif
     ...
     switch (region->config.buffermode) {
          ...
          case DLBM_BACKVIDEO:
               ...
               {
#ifndef BACK_FRONT_CHANGE
                    dfb_gfx_copy_regions( region->surface, CSBR_FRONT, region->surface, CSBR_BACK, updates, num_updates, 0, 0 );
#else
                    if(dfb_config->qt_flip)
                    {
					  dfb_config->copy_region.x1 = updates->x1;
					  dfb_config->copy_region.x2 = updates->x2;
					  dfb_config->copy_region.y1 = updates->y1;
					  dfb_config->copy_region.y2 = updates->y2;
                      dfb_config->qt_flip = false;
                    }
                    else
                      dfb_gfx_copy_regions( region->surface, CSBR_FRONT, region->surface, CSBR_BACK, updates, num_updates, 0, 0 );  
#endif
