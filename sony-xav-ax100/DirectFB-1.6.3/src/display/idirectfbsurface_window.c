...
#ifdef check_swap
#include <sys/ioctl.h>
#define FBIO_QUERYUPDATE	_IOW('F', 0x21, u_int32_t)
#endif
                              ...
                              DFBSurfaceFlipFlags  flags )
{
   ...
   dfb_config->qt_flip = true;

  if(dfb_config->qt_flip_in == 1)
    dfb_config->qt_flip_in=2;
  else if(dfb_config->qt_flip_in == 0)
    dfb_config->qt_flip_in = 1;
#endif

#ifdef FPS_PRINT
  if(dfb_config->fps_print)
  {
      if( (_getNowSec() - _sec) >= 1 ) {
          _sec = _getNowSec();
          printf("[DirectFB] %d FPS\n", _flush);
          _flush = 0;
      }
      else
      {
...
#ifdef check_swap
		  int tmp;
		  ioctl( dfb_config->fbNum, FBIO_QUERYUPDATE,  &tmp);
#endif
