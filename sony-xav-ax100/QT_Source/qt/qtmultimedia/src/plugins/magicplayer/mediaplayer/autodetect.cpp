...
static const CharRange kKIORRanges[] = {
    { 0x00A1, 0x00FF },
  
};

static const CharRange kTISRanges[] = {
    { 0x00A1, 0x00DA },
	{ 0x00DF, 0x00FB }, 
};
...
extern uint32_t findPossibleEncoding(int ch)
{
    ...
    if (ch < 128) return kEncodingAll;	
	 int result = kEncodingNone;
	if (charMatchesEncoding(ch,kKIORRanges,ARRAY_SIZE(kKIORRanges)))
		result |= kEncodingKIOR;
	if (charMatchesEncoding(ch,kTISRanges,ARRAY_SIZE(kTISRanges)))
		result |= kEncodingTIS;
    return result;
}
