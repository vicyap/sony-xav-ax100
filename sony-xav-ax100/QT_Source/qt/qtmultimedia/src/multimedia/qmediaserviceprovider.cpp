...
public:
    QMediaService* requestService(const QByteArray &type, const QMediaServiceProviderHint &hint)
    {
        ...
        if (!plugins.isEmpty()) {
            ...
            if (plugin != 0) {
				if(hint.features()&QMediaServiceProviderHint::OnlyMetaData)
				{
					key += QLatin1String("QMediaServiceProviderHint::OnlyMetaData");
				}else if(hint.features()&QMediaServiceProviderHint::VideoThumbnail)
				{
					key += QLatin1String("QMediaServiceProviderHint::VideoThumbnail");
				}
                ...
                if (iface) {
                    ...
                    if ((flags & QMediaPlayer::StreamPlayback) &&
                        !(features & QMediaServiceProviderHint::StreamPlayback))
                            continue;
					//only open metadata,can't play
					if ((flags & QMediaPlayer::OnlyMetaData) &&
                        !(features & QMediaServiceProviderHint::OnlyMetaData))
                            continue;

					//video thumbnail support
					if ((flags & QMediaPlayer::VideoThumbnail) &&
                        !(features & QMediaServiceProviderHint::VideoThumbnail))
                            continue;
                }
            }
