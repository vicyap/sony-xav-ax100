...
static void qRegisterMediaPlayerMetaTypes()
{
	...
	qRegisterMetaType<QMediaPlayer::ContainerType>("QMediaPlayer::ContainerType");
	qRegisterMetaType<QMediaPlayer::AudioCodecType>("QMediaPlayer::AudioCodecType");
	qRegisterMetaType<QMediaPlayer::VideoCodecType>("QMediaPlayer::VideoCodecType");
	qRegisterMetaType<QMediaPlayer::MediaMessage>("QMediaPlayer::MediaMessage");
	qRegisterMetaType<QMediaPlayer::MediaInvoke>("QMediaPlayer::MediaInvoke");
	qRegisterMetaType<QMediaPlayer::ResumeData>("QMediaPlayer::ResumeData");
	#endif
}
...
static QMediaService *playerService(QMediaPlayer::Flags flags)
{
    ...
    if (flags) {
		...
		if (flags & QMediaPlayer::OnlyMetaData)
            features |= QMediaServiceProviderHint::OnlyMetaData;
                 ...
                 playerService(flags))
{
    ...
    if (d->service == 0) {
		...
		#ifdef SUPPORT_SUNPLUS_EXTEND
			connect(d->control, SIGNAL(mediaMessageChanged(QMediaPlayer::MediaMessage,QVariant)), SIGNAL(mediaMessageChanged(QMediaPlayer::MediaMessage,QVariant)));;
		#endif
...
#ifdef SUPPORT_SUNPLUS_EXTEND
void QMediaPlayer::invoke(QMediaPlayer::MediaInvoke invokeid,QVariant invokeinfo) 
{
    Q_D(const QMediaPlayer);

    if (d->control != 0)
       d->control->invoke(invokeid,invokeinfo);
