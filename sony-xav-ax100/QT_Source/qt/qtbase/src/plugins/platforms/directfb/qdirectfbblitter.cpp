...
void QDirectFbBlitter::alphaFillRect(const QRectF &rect, const QColor &color, QPainter::CompositionMode cmode)
{
	...
	else if (cmode == QPainter::CompositionMode_SourceOver)
	{
	...
	else 
	{
        ...
        // check if operation is useless
        //DENNIS MARK FOR DFB FILLRECT
        if (color.alpha() == 0 && cmode != QPainter::CompositionMode_Clear)
            return;
		...
		//DENNIS MARK FOR DFB FILLRECT
        if(cmode == QPainter::CompositionMode_Clear)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_CLEAR);
		else if(cmode == QPainter::CompositionMode_Source)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_SRC);
		else if(cmode == QPainter::CompositionMode_DestinationOver)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_DST_OVER);
		else if(cmode == QPainter::CompositionMode_SourceIn)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_SRC_IN);
		else if(cmode == QPainter::CompositionMode_DestinationIn)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_DST_IN);
		else if(cmode == QPainter::CompositionMode_SourceOut)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_SRC_OUT);
		else if(cmode == QPainter::CompositionMode_DestinationOut)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_DST_OUT);
		else if(cmode == QPainter::CompositionMode_SourceAtop)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_SRC_ATOP);
		else if(cmode == QPainter::CompositionMode_DestinationAtop)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_DST_ATOP);
		else if(cmode == QPainter::CompositionMode_Plus)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_ADD);
		else if(cmode == QPainter::CompositionMode_Xor)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_XOR);
		else if(cmode == QPainter::CompositionMode_Destination)
			m_surface->SetPorterDuff(m_surface.data(), DSPD_DST);
		else
	        m_surface->SetPorterDuff(m_surface.data(), DSPD_NONE);
    }
