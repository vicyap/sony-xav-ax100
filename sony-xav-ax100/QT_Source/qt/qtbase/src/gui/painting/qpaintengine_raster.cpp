                                          ...
                                          const QFixedPoint *positions, QFontEngine *fontEngine)
{
    ...
    if (fontEngine->hasInternalCaching()) {
        ...
        if(fontEngine->fontDef.spStroke){ //S+
            neededFormat = QFontEngine::Format_A32;
        }
        ...
        for (int i = 0; i < numGlyphs; i++) {
            ...
            if(fontEngine->fontDef.spStroke){ //S+
                ...
                s->matrix = QTransform();
                drawImage(QPoint(qFloor(positions[i].x) + offset.x(), qRound(positions[i].y) + offset.y()), *alphaMap);
