...
static const QCssKnownValue properties[NumProperties - 1] = {
    ...
    { "spacing", QtSpacing },
    { "stroke", SpStroke },    //S+
    { "strokecolor", SpStrokeColor },    //S+
    { "strokelinecolor", SpStrokeLineColor },	 //S+
...
static void spSetFontStroke(const QCss::Value &value, QFont *font)
{
    int v = value.variant.toInt();
    int stroke = font->spStroke();
    //qDebug("%s %d type = %d v = %d ",  __FUNCTION__, __LINE__, value.type, v);

    if (value.type != Value::KnownIdentifier)
        return;

    if (v == Value_On) {
        font->spSetStroke(true);
    }else if (v == Value_Off){
        font->spSetStroke(false);
    }
}

static void spSetFontStrokeColor(const QCss::Value &value, QFont *font)
{
    //qDebug("%s %d type = %d  ",  __FUNCTION__, __LINE__, value.type);
    if (value.type != Value::Color)
        return;

    ColorData c = parseColorValue(value);
    uint color = (uint)c.color.rgba();
    font->spSetStrokeColor(color);
}

static void spSetFontStrokeLineColor(const QCss::Value &value, QFont *font)
{
    //qDebug("%s %d type = %d  ",  __FUNCTION__, __LINE__, value.type);
    if (value.type != Value::Color)
        return;
    ...
    ColorData c = parseColorValue(value);
    uint color = (uint)c.color.rgba();
    font->spSetStrokeLineColor(color);
}
...
bool ValueExtractor::extractFont(QFont *font, int *fontSizeAdjustment)
{
    ...
    for (int i = 0; i < declarations.count(); ++i) {
        ...
        switch (decl.d->propertyId) {
            ...
            //S+
            case SpStroke: spSetFontStroke(val, font); break;
            case SpStrokeColor: spSetFontStrokeColor(val, font); break;
            case SpStrokeLineColor: spSetFontStrokeLineColor(val, font); break;
