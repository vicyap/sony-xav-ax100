...
//S+
#define Q_FT_BORDER_SIZE          1
#define Q_FT_STROKE_EXTRA_SIZE    (Q_FT_BORDER_SIZE * 2)
#define Q_FT_STROKE_EXTRA_WIDTH   0 //default:Q_FT_STROKE_EXTRA_SIZE
#define Q_FT_STROKE_GLYPH_UNIT    6
#define Q_FT_STROKE_GLYPH_BORDER  (Q_FT_BORDER_SIZE << Q_FT_STROKE_GLYPH_UNIT) //for glyph: 1 pixel is 64 units
#define Q_FT_STROKE_ADJUST_PARA   (1.2)    //make small font inside clear
#define Q_FT_STROKE_SMALL_HEIGHT  15       //under 15 pixel is small font
//#ifdef THAI_SCALE_DOWN
#define Q_FT_SCALE_DOWN_RATIO     (0.72)      //up, down 0.1, lef right 0.1
//#endif

int  styleCount;
ARGBColor defaultStrokeColor(0xFF, 0xFF, 0xFF); //default: white rgba
ARGBColor defaultStrokeLineColor(0x00, 0x00,0x00); //default: black rgba
ARGBColor gStrokeColor(defaultStrokeColor.r, defaultStrokeColor.g, defaultStrokeColor.b); //draw: white rgba
ARGBColor gStrokeLineColor(defaultStrokeLineColor.r, defaultStrokeLineColor.g,defaultStrokeLineColor.b); //draw: black rgba
static void draw_font_bitmap(uchar *buf, int pitch, int len);
...
//S+
static void convertGRAYToStrokeARGB(const uchar *isrc, const uchar *src, uint *dst, QFontEngineFT::GlyphInfo* info, int src_pitch, uint stroke, uint outline, bool isSmall) {
    //qDebug("convertGRAYToStrokeARGB width = %d height = %d, src_pitchs = %d", info.width, info.height, src_pitch);
    //qDebug("convertGRAYToStrokeARGB gStrokeColor = 0x%x, gStrokeLineColor = 0x%x", stroke, outline);
    if(stroke == 0 && outline == 0){ //not set color, use default
        gStrokeColor.setColor(defaultStrokeColor);
        gStrokeLineColor.setColor(defaultStrokeLineColor);
    }else{
        gStrokeColor.setColor(stroke);
        gStrokeLineColor.setColor(outline);
    }

    for (int y = 0; y < info->height; ++y) {
        int readpos =  (y * src_pitch);
        int writepos = (y * info->width);
        for (int x = 0; x < info->width; ++x) {
            if (!src[readpos + x] && !isrc[readpos + x]){
                dst[writepos + x] = 0x00000000; //0x00...... transparent
            }else if(src[readpos + x] && !isrc[readpos + x]){
                dst[writepos + x] = (src[readpos + x] << 24) + (gStrokeLineColor.r << 16) + (gStrokeLineColor.g << 8) + gStrokeLineColor.b;
            }else{
                uchar inside = isrc[readpos + x];
				uchar outline = (0xFF - inside);
                if(isSmall){ //set inside alpha higher would make the color more bright
					inside = ((isrc[readpos + x] * Q_FT_STROKE_ADJUST_PARA) > 0xFF) ? 0xFF: (isrc[readpos + x] * Q_FT_STROKE_ADJUST_PARA);
					outline = ((0xFF - inside) > 0x7F) ? 0x7F : (0xFF - inside);
                }
                int r = ((gStrokeColor.r*inside)>>8) + ((gStrokeLineColor.r*outline)>>8);
                int g = ((gStrokeColor.g*inside)>>8) + ((gStrokeLineColor.g*outline)>>8);
                int b = ((gStrokeColor.b*inside)>>8) + ((gStrokeLineColor.b*outline)>>8);
                dst[writepos + x] = (0xFF << 24) + (r << 16) + (g << 8) + b;
            }
        }
    }
}
    ...
    : QFontEngine(Freetype)
{
    ...
    //S+
    spstroke = fd.spStroke;
    if(spstroke){
        spstrokecolor = (fd.spStrokeColor & 0xFFFFFF);
        spstrokelinecolor = (fd.spStrokeLineColor & 0xFFFFFF);
    }else{
        spstrokecolor = 0x00000000;
        spstrokelinecolor = 0x00000000;
    }
//#ifdef THAI_SCALE_DOWN
    bool spscaledown = bool(fontDef.family.contains(QLatin1String("Thai"), Qt::CaseInsensitive));

    if(spscaledown){
    	  QString percentage = QString::fromLocal8Bit(qgetenv("QT_THAI_SHRINK"));
    	  if(percentage.length() > 0){
            fontDef.pixelSize = (fontDef.pixelSize * percentage.toInt() / 100);
            qDebug() << " Thai Shrink QT_THAI_SHRINK = " << percentage;
    	  }else{
            fontDef.pixelSize = (fontDef.pixelSize * Q_FT_SCALE_DOWN_RATIO);
            qDebug() << " Thai Shrink default = " << Q_FT_SCALE_DOWN_RATIO;
        }
                         ...
                         QFreetypeFace *freetypeFace)
{
    ...
    */
    ...
    //S+
    spstroke = fontDef.spStroke;
    //qDebug("QFontEngineFT::init %d 0x%x 0x%x", fontDef.spStroke,fontDef.spStrokeColor,fontDef.spStrokeLineColor);

    if(spstroke){
        spstrokecolor = (fontDef.spStrokeColor & 0xFFFFFF);
        spstrokelinecolor = (fontDef.spStrokeLineColor & 0xFFFFFF);
    }else{
        spstrokecolor = 0x00000000;
        spstrokelinecolor = 0x00000000;
    }
...
//     Q_ASSERT(freetype->lock == 1);
	QVariant px(fontDef.pixelSize);
	bool isSmall = (px.toInt() < Q_FT_STROKE_SMALL_HEIGHT) ? true:false;
    ...
    //S+ enlarge size
    if(spstroke && !isSmall){
        info.x += Q_FT_BORDER_SIZE;
        info.y += Q_FT_BORDER_SIZE;
        info.width += Q_FT_STROKE_EXTRA_SIZE;
        info.height += Q_FT_STROKE_EXTRA_SIZE;
    }
    ...
    if (slot->format == FT_GLYPH_FORMAT_OUTLINE) {
        ...
        if (spstroke){
            ...
            FT_Outline ooutline;
            FT_BBox    cbox;
            int        borderValue = isSmall? (Q_FT_STROKE_GLYPH_BORDER>>1):Q_FT_STROKE_GLYPH_BORDER; // if small font, half border

            FT_Stroker_New(library, &stroker);
            FT_Stroker_Set(stroker, borderValue, FT_STROKER_LINECAP_ROUND,FT_STROKER_LINEJOIN_ROUND,0);
            FT_Stroker_ParseOutline(stroker, &slot->outline, 0);
            FT_Stroker_GetCounts(stroker, &opoints, &ocontours);
            FT_Outline_New(library, opoints, ocontours, &ooutline);
            ooutline.n_points = 0;
            ooutline.n_contours = 0;

            //FT_Stroker_ExportBorder(stroker, FT_STROKER_BORDER_LEFT , &ooutline);
            FT_Stroker_Export(stroker, &ooutline);

            //out stroke
            FT_Outline_Get_CBox(&ooutline, &cbox);

			ibuf = new uchar[bitmap.rows*bitmap.pitch];
			memset(ibuf, 0, bitmap.rows*bitmap.pitch);

            /*because the bottom of each char are different, ex: y and z
              we can not shift the glyph by cbox.yMin but the stroke outline width
            */
            if(isSmall){
                uchar *temp = bitmap.buffer;

                FT_Outline_Get_Bitmap(library, &ooutline, &bitmap);
                bitmap.buffer = ibuf;

                FT_Outline_Get_Bitmap(library, &slot->outline, &bitmap);
                bitmap.buffer = temp;
            }else{
                FT_Outline_Translate (&ooutline, Q_FT_STROKE_GLYPH_BORDER, Q_FT_STROKE_GLYPH_BORDER);
                FT_Outline_Get_Bitmap(library, &ooutline, &bitmap);
                //in
                initRenderUser(&iusr, &bitmap, ibuf);
                FT_Outline_Translate (&slot->outline, Q_FT_STROKE_GLYPH_BORDER, Q_FT_STROKE_GLYPH_BORDER);				
                renderSpans(library, &slot->outline, &iusr, (FT_SpanFunc)iRasterCallback);
            }
            //debug log
            //draw_font_bitmap(ibuf, bitmap.pitch, bitmap.rows*bitmap.pitch);
            //draw_font_bitmap(bitmap.buffer, bitmap.pitch, bitmap.rows*bitmap.pitch);

            FT_Outline_Done(library, &ooutline);
        ...
        if (hsubpixel) {
            ...
            if(spstroke){ //S+
                convertGRAYToStrokeARGB(ibuf, bitmap.buffer, (uint *)glyph_buffer, &info, bitmap.pitch, spstrokecolor, spstrokelinecolor, isSmall);
            }else{
                convertGRAYToARGB(bitmap.buffer, (uint *)glyph_buffer, info.width, info.height, bitmap.pitch);
            }
        }

        if (ibuf) delete [] ibuf; //S+
...
bool QFontEngineFT::initFromFontEngine(const QFontEngineFT *fe)
{
    ...
    spstroke = fe->spstroke;
    spstrokecolor = fe->spstrokecolor;
    spstrokelinecolor = fe->spstrokelinecolor; 
...
//S+
void iRasterCallback(const int y, const int count, const FT_Span * const spans, void * const user) 
{
    RenderUser *info = (RenderUser *)user;
    FT_Bitmap *bmp = info->bmp;
    uchar* output = info->output;

    for  (int i = 0; i < count; ++i){
        for (int w = 0; w < spans[i].len ; ++w){
            int ypos = (int)(bmp->rows - 1 - y);
            int xpos = (int)(spans[i].x + w);
            int pos = ypos * bmp->pitch + xpos;
            output[pos] = spans[i].coverage;
        }
    }
}

//S+
void initRenderUser(RenderUser* s, FT_Bitmap *bmp, uchar *output){
    s->bmp = bmp;
    s->output = output;
}

//S+
void renderSpans(FT_Library &library, FT_Outline * const outline, RenderUser * usr, FT_SpanFunc callback) 
{
...
static void draw_font_bitmap(uchar *buf, int pitch, int len){
    ...
    for(int b=0; b < len; ){
        memset(raw, 0, size);
        ptr = raw;
        for(int now = 0; now < pitch; now ++){
            sprintf(ptr, "%3d ", buf[b++]);
            ptr += 4;
        }
        qDebug("%s", raw);
    }
