...
bool QFontDef::exactMatch(const QFontDef &other) const
{
    ...
    */
    ...
    //S+
    bool strokeMatch = false;
    if(spStroke){
        strokeMatch = (spStroke == other.spStroke) && (spStrokeColor == other.spStrokeColor) && (spStrokeLineColor == other.spStrokeLineColor);
    }else{
        strokeMatch = (spStroke == other.spStroke);
    }
...
void QFontPrivate::resolve(uint mask, const QFontPrivate *other)
{
    ...
    //S+
    if (! (mask & QFont::SpStrokeResolved))
        request.spStroke = other->request.spStroke;
    if (! (mask & QFont::SpStrokeColorResolved))
        request.spStrokeColor = other->request.spStrokeColor;
    if (! (mask & QFont::SpStrokeLineColorResolved))
        request.spStrokeLineColor = other->request.spStrokeLineColor;
}
...
//S+
bool QFont::spStroke() const
{
    return d->request.spStroke;
}

//S+
void QFont::spSetStroke(bool isEnable)
{
    //qDebug("%s %d value = %d resolve_mask = %d",  __FUNCTION__, __LINE__, isEnable, resolve_mask);

    if ((resolve_mask & QFont::SpStrokeResolved) && d->request.spStroke == isEnable)
        return;

    detach();

    d->request.spStroke = isEnable;
    if(isEnable){
        if(!(resolve_mask & QFont::SpStrokeColorResolved))
            d->request.spStrokeColor = 0;
        if(!(resolve_mask & QFont::SpStrokeLineColorResolved))
            d->request.spStrokeLineColor = 0;     
    }

    resolve_mask |= QFont::SpStrokeResolved;
}

//S+
uint QFont::spStrokeColor() const
{
    return d->request.spStrokeColor;
}

//S+
void QFont::spSetStrokeColor(uint color)
{
    //qDebug("%s %d value = 0x%x resolve_mask = %d",  __FUNCTION__, __LINE__, color, resolve_mask);

    if ((resolve_mask & QFont::SpStrokeColorResolved) && d->request.spStrokeColor == color)
        return;

    detach();

    d->request.spStrokeColor = color;
    resolve_mask |= QFont::SpStrokeColorResolved;
}

//S+
uint QFont::spStrokeLineColor() const
{
    return d->request.spStrokeLineColor;
}

//S+
void QFont::spSetStrokeLineColor(uint color)
{
    //qDebug("%s %d value = 0x%x resolve_mask = %d",  __FUNCTION__, __LINE__, color, resolve_mask);

    if ((resolve_mask & QFont::SpStrokeLineColorResolved) && d->request.spStrokeLineColor == color)
        return;

    detach();

    d->request.spStrokeLineColor = color;
    resolve_mask |= QFont::SpStrokeLineColorResolved;
}
