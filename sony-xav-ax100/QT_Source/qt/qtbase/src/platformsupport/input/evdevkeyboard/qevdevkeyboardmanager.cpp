    ...
    : QObject(parent)
{
    ...
    QString IR_node = QString::fromLocal8Bit(qgetenv("QT_CUSTOMIZE_IR_DEVICE"));
    if(!IR_node.isEmpty()){
        IR_spec = QString::fromLocal8Bit(qgetenv("QT_CUSTOMIZE_EVDEV_IR_PARAMETERS"));
        addIrRemote(IR_node);
    }
...
void QEvdevKeyboardManager::addIrRemote(const QString &deviceNode)
{
#ifdef QT_QPA_KEYMAP_DEBUG
    qWarning() << "addIrRemote" << deviceNode;
#endif

    QEvdevKeyboardHandler *keyboard;
    keyboard = QEvdevKeyboardHandler::create(deviceNode, IR_spec);
    if (keyboard)
        m_keyboards.insert(deviceNode, keyboard);
    else
        qWarning("Failed to addIrRemote");
}
