...
#ifdef  USB_TRY_EHCI
static int speed_reset[3] = {1,1,1};
#endif
...
#ifdef CONFIG_USB_HOST_RESET_SP_
{
	...
	if((hcd->self.root_hub->speed == USB_SPEED_HIGH) && (hcd->self.root_hub->speed != udev->speed))
		return;
	...
	//printk("<<<######>>%d %d %p %p %ld\n",hcd->driver->hcd_priv_size,sizeof(wait_queue_head_t),ptr,ptr + sizeof(wait_queue_head_t)/4,udev->bus->devmap.devicemap[0]);
...
int usb_mirrorlink_configuration(struct usb_device *dev, int configuration)
{
	...
	//ADATA udisk vid 0x125f pid 0x312b,will stall & reset ,not send mirrorlink cmd
	if((dev->descriptor.idProduct == 0x312b) &&  (dev->descriptor.idVendor == 0x125f))
		return 0;

	ret = usb_control_msg(dev, usb_sndctrlpipe(dev, 0),
			      0xF0, 0x40, cpu_to_be16(MIRROR_LINK_VER), cpu_to_be16(HOST_ID),
			      NULL, 0, 1000);
...
static int usb_logo_thread(void *arg)
{
	...
	printk("idVendor = 0x%x idProduct= 0x%x 0x%x 0x%x\n", idVendor, idProduct, udev->descriptor.idVendor,udev->descriptor.idProduct);
	...
	if ( (idVendor == 0x1A0A) && (
		...
		hub_port_reset(hub, udev->portnum, udev, HUB_SHORT_RESET_TIME, false);
...
#define GET_DESCRIPTOR_BUFSIZE	64
...
#ifdef CONFIG_RETRY_TIMES
					if((hdev->speed == USB_SPEED_HIGH) && (r < 0) && (j == 2)) {
						printk("Waring: down to full speed\n");
						retval = -ENOTCONN;
						udev->reset_count = 0;
						kfree(buf);
						goto fail;
			...
			if (r) {
				...
				if (status&USB_PORT_STAT_CONNECTION) {
					udev->descriptor.bDeviceClass = USB_CLASS_DEVICE_NOT_SUPPORT;
					udev->speed = USB_SPEED_HIGH;
				}
#endif
				retval = -EMSGSIZE;
...
#ifdef  USB_TRY_EHCI
{
		...
		//printk("--(----- %d %d %d\n",udev->speed,hcd->self.busnum,speed_reset[hcd->self.busnum - 4]);
		if(test_bit(USB_EHCI_LOADED, &usb_hcds_loaded) 			\
						&& (udev->level == 1 )) {
			if(speed_reset[hcd->self.busnum - 4] == 1){
				printk("Retry enumerate device by EHCI %x!!!\n", udev->speed);
				Reset_Usb_Power(hcd->self.busnum - 1);
				speed_reset[hcd->self.busnum - 4] = 2;
			}
			else if(speed_reset[hcd->self.busnum - 4] == 2){
				printk("Run in OHCI !!!\n");
				speed_reset[hcd->self.busnum - 4] = 1;
			}
			else
				speed_reset[hcd->self.busnum - 4]++;
			}
		}
#endif
					...
					u16 portstatus, u16 portchange)
{
	...
	for (i = 0; i < SET_CONFIG_TRIES; i++) {
		...
		if(hcd->self.busnum < 4){
			speed_reset[hcd->self.busnum - 1] = 1;
		}
#endif

#ifdef CONFIG_USB_LOGO_TEST
		hub->usb_logo_thread = kthread_create(usb_logo_thread, udev, "usb_logo_test");
