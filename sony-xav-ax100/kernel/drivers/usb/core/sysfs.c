...
#else
usb_actconfig_attr(bNumInterfaces, 1, "%d\n")
#endif
		...
		char *buf)
{
	...
	if (actconfig)
		...
		for(i = 0; i < udev->descriptor.bNumConfigurations; i++)
			maxp = maxp > udev->config[i].desc.bMaxPower ? maxp :
...
/* "Get a device status" */
static ssize_t
show_usb_device_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct usb_device *udev;
