...
#if 0
						struct list_head *td_entry, *td_next;
						struct td *td_tmp;
						n++;

	                    list_for_each_safe (td_entry, td_next, &ed->td_list) {
	                    	td_tmp = list_entry (td_entry, struct td, td_list);
							printk("ed %08x %08x %08x %08x %08x/%08x | td_%d [%08x]: %08x %08x %08x %08x\n",
											head, ed->hwINFO, ed->hwTailP, ed->hwHeadP, ed->hwNextED,
											td->td_dma,
											td->index, td_tmp->td_dma,
											td_tmp->hwINFO, td_tmp->hwCBP,
											td_tmp->hwNextTD, td_tmp->hwBE
											);
	                     }
#endif
						goto skip_ed;
...
dl_done_list (struct ohci_hcd *ohci)
{
	...
	while (td) {
		...
		for(;;) {
			...
			td2 = list_first_entry(&ed->td_list,struct td, td_list);
