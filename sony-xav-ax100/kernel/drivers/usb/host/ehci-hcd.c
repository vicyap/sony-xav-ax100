...
static void timer_action(struct ehci_hcd *ehci, enum ehci_timer_action action)
{
...
static int handshake_on_error_set_halt(struct ehci_hcd *ehci,
				       void __iomem * ptr, u32 mask, u32 done,
				       int usec)
{
	int error;
...
static inline int ehci_get_platform_id(struct ehci_hcd *ehci)
{
...
#define DEV_CONNECT_STA	((3 << 10)|PORT_CONNECT)

static void may_hcd_daed(struct ehci_hcd *ehci)
{
	struct usb_hcd *hcd;
	int no_dev;
	u32 ehci_port;
	...
	no_dev = is_usb_bus_empty(hcd_to_bus(hcd));
	ehci_port = ehci_readl(ehci, &ehci->regs->port_status[0]);

	if(((ehci_port & DEV_CONNECT_STA) == 0) && (no_dev == 0)) {
		printk("-.- may ehci daed\n");
...
static int ehci_init(struct usb_hcd *hcd)
{
	...
	if (HCC_LPM(hcc_params)) {
		...
		ehci->has_lpm = 1;
		if (hird > 0xf) {
			ehci_dbg(ehci, "hird %d invalid, use default 0", hird);
			hird = 0;
		}
		temp |= hird << 24;
	}
...
static irqreturn_t ehci_irq(struct usb_hcd *hcd)
{
	...
	if (pcd_status) {
		...
		port_id = ehci_get_platform_id(ehci) - 1;
		port_status = ehci_readl(ehci, &ehci->regs->port_status[0]);

		//printk("usb device disconnect %x %x\n", port_status, dev_instered);

		if((port_status&PORT_CONNECT) == 0) {
			if(Get_uphy_owner(port_id)&&((port_status & (3<<10)) == 0) && (dev_instered & (1 << port_id))){
				*(hcd->ptr_flag) |=  (RESET_HC_DEAD | RESET_UPHY_SIGN);
				wake_up_interruptible(&hcd->reset_queue);
			}

			dev_instered &= ~(1 << port_id);
		}
		else {
			if(((port_status >> 10)&0x03) == 0x2)
				dev_instered |= (1 << port_id);
		}
#endif
 ...
 */
static int ehci_urb_enqueue(struct usb_hcd *hcd,
...
static void unlink_async(struct ehci_hcd *ehci, struct ehci_qh *qh)
{
	...
	if (ehci->reclaim) {
		...
		for (last = ehci->reclaim; last->reclaim; last = last->reclaim)
			continue;
...
int USB_transfer(struct ehci_hcd *ehci, struct ehci_qtd *qtd,
...
static int Ehci_Host_Setup(struct ehci_hcd *ehci)
{
	...
	return USB_transfer(ehci, qtd_setup, setup_dma, 8);
}

static int Ehci_Host_In(struct ehci_hcd *ehci)
{
	printk(">>> Ehci_Host_In...\n");
	...
	return USB_transfer(ehci, qtd_in, in_dma, transfer_lenth);
}
static int Ehci_Host_Out(struct ehci_hcd *ehci)
{
	...
	return USB_transfer(ehci, qtd_out, out_dma, 0);
}
static unsigned int Test_PORT_EHCI(struct ehci_hcd *ehci)
{
	...
	do {
		...
		msleep(15000);	// wait 15 seconds

		//4  resume
		Temp = ehci_readl(ehci, &ehci->regs->port_status[0]);
		Temp |= PORT_RESUME;
		ehci_writel(ehci, Temp, &ehci->regs->port_status[0]);
		msleep(25);	// wait 25 ms, minimum is 20 ms
...
static int ehci_usb_logo_test(struct usb_hcd *hcd, int idProduct)
{
	...
	volatile u32 *grop_base =
	    (u32 *) IO0_ADDRESS((148 + hcd->self.busnum) * 32 * 4);
