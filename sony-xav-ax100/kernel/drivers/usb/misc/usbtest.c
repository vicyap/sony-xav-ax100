...
#include <linux/usb.h>
static u32 check_pattern=0;
static unsigned force_in_out=0;
module_param(force_in_out, uint, S_IRUGO | S_IWUSR);
...
/*-------------------------------------------------------------------------*/
void PrintBlock_usb_test(unsigned char* pBuffStar, unsigned int uiBuffLen)
{
	unsigned int uiIter;
	pBuffStar = pBuffStar;
	printk("pBuffStar=%p\n", pBuffStar);
...
get_endpoints(struct usbtest_dev *dev, struct usb_interface *intf)
{
	...
	struct usb_host_endpoint	*in, *out,*int_in;
	...
	for (tmp = 0; tmp < intf->num_altsetting; tmp++) {
		...
		for (ep = 0; ep < alt->desc.bNumEndpoints; ep++) {
			...
			case USB_ENDPOINT_XFER_ISOC:
				printk("get iso ep!\n");
				if (ep==force_in_out||dev->info->iso){
					printk("iso ep!\n");
					goto try_iso;
				}
				/* FALLTHROUGH */
...
over:		if ((in && out)  ||  iso_in || iso_out || int_in)
			goto found;
	}
	return -EINVAL;
...
found:
	...
	if (iso_in) {
		...
		dev->in_iso_pipe = usb_rcvisocpipe(udev,5);
		printk("in %s %d \n",__FUNCTION__,__LINE__);
	}
	...
	if (iso_out) {
		...
		dev->out_iso_pipe = usb_sndisocpipe(udev,
				iso_out->desc.bEndpointAddress
					& USB_ENDPOINT_NUMBER_MASK);
		//dev->out_iso_pipe = usb_sndisocpipe(udev,12);
		printk("in %s %d \n",__FUNCTION__,__LINE__);
	}
	//dev->int_pipe = usb_rcvintpipe(udev,3);
	printk("in %s %d \n",__FUNCTION__,__LINE__);
	return 0;
}
...
static inline void simple_fill_buf(struct urb *urb)
{
	...
	case 1:		
		#if 0
		/* mod63 */
		for (i = check_pattern; i < len+check_pattern; i++)
			*buf++ = (u8) (i % 63);
		check_pattern++;
		#else
		p = (u32 *)buf;
...
static int simple_check_buf(struct usbtest_dev *tdev, struct urb *urb)
{
	...
	if (ret)
		return ret;
	ERROR(tdev, "simple_check len %d buf0 %x pend %x\n", len, p[0],p[(len)/4 - 1]);
	...
	for (i = 0; i < (len)/4; i++, p++) {
		 ...
		 */
		case 1:			/* mod63 */
			//expected = (i+ *((u8*)urb->transfer_buffer)) % 63;
			expected = p0 + (u32)i;
			break;
		...
		if (*p == expected)
			continue;
		ERROR(tdev, "buf[%d] = %x (not %x)\n", i, *p, expected);
		PrintBlock_usb_test(buf,len);
		return -EINVAL;
	}
	return 0;
}
...
alloc_sglist(int nents, int max, int vary)
{
	...
	for (i = 0; i < nents; i++) {
		...
		case 1:
			...
			p = (u32 *)buf;
			t = (size + 3) / 4;
			for(j = check_pattern; j < check_pattern + t;j++)
				*p++ = (u32)j;
			//check_pattern++;
			check_pattern = j;
			#endif
...
static int set_altsetting(struct usbtest_dev *dev, int alternate)
{
	...
	#if 1  //for test iso set intf 4 alt 5
	return usb_control_msg(udev, usb_sndctrlpipe(udev, 0),
				   USB_REQ_SET_INTERFACE, USB_RECIP_INTERFACE,
				   (interface>>8)&0xf, interface&0xf, NULL, 0, USB_CTRL_SET_TIMEOUT);
	#endif
...
)
{
	...
	memset(urb->transfer_buffer,
			usb_pipein(urb->pipe) ? GUARD_BYTE : 0,
			bytes);
	memset(urb->transfer_buffer,GUARD_BYTE ,bytes);
	for (i = 0; i < packets; i++) {
...
printk("%s %d %d %d\n",__FUNCTION__,__LINE__,bytes,usb_pipein(urb->pipe));
	u8 * buf = urb->transfer_buffer;
	for (i = check_pattern; i < bytes+check_pattern; i++)
		*buf++ = (u8) (i % 63);
	check_pattern++;
...
{
	printk("int data: \n");
	struct int_context	*ctx = urb->context;
	int i;
	u8		*buf = urb->transfer_buffer;
	for(i=0;i<8;i++)
		printk("0x%x ",buf[i]);
...
static int
test_int(struct usbtest_dev *dev, struct usbtest_param *param,
		int pipe,  int inter, unsigned offset)
{
	struct int_context	context;
	struct usb_device	*udev;
	unsigned		i;
	unsigned long		packets = 0;
	int			status = 0;
	...
	while(param->iterations--){
		printk("%d iterations = %d \n",__LINE__,param->iterations);
		context.int_urb = usb_alloc_urb(0, GFP_ATOMIC);
		if(!context.int_urb)
			return context.int_urb;
		udev = testdev_to_usbdev(dev);
		context.int_urb->context = &context;
		context.data = usb_alloc_coherent(udev, 8, GFP_ATOMIC, &context.data_dma);
		if(!context.data)
			printk("context.data faied \n");
		usb_fill_int_urb(context.int_urb, udev, pipe, context.data,
				 8 ,usb_mouse_irq, &context, 4);
		context.int_urb->transfer_dma = context.data_dma;
		context.int_urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
		printk("%d here pipe %d in_pipe %d\n",__LINE__,pipe,dev->in_pipe);
		status = usb_submit_urb(context.int_urb, GFP_KERNEL);
		printk("%d here status =%d\n",__LINE__,status);
		wait_for_completion(&context.done);
		printk("%d here status =%d\n",__LINE__,status);
		status = context.int_urb->status;
		printk("%d here %d\n",__LINE__,status);
		if(status < 0)
			;//return status;
	}
	...
	memset(context.int_urb, 0, sizeof context.int_urb);
	udev = testdev_to_usbdev(dev);
	printk("%d here \n",__LINE__);
	...
	//packets *= param->iterations;
	printk("%d here \n",__LINE__);
	context.data = kzalloc(8,GFP_ATOMIC);

	usb_fill_int_urb(context.int_urb, udev, pipe, context.data,
			 8 ,usb_mouse_irq, &context, 8);
	
	context.int_urb->transfer_dma = context.data_dma;
	context.int_urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
	printk("%d here \n",__LINE__);
	spin_lock_irq(&context.lock);
	status = usb_submit_urb(context.int_urb, GFP_ATOMIC);
	if (status < 0) {
		ERROR(dev, "submit int, error %d\n",status);
		//context.pending--;
		context.submit_error = 1;
		spin_unlock_irq(&context.lock);
		return status;
...
usbtest_ioctl(struct usb_interface *intf, unsigned int code, void *buf)
{
	...
	#if 0 //iso test set intf 4 alt 5
	//printk("set interface 4 alt 5 for iso \n");
	int	res;
	...
	case 2:
		...
		dev_info(&intf->dev,
				"TEST 2:  read %d bytes %u times\n",
				param->length, param->iterations);
		test_int(dev, param,dev->in_pipe,8, 0);
#endif
		break;
	case 3:
	...
	case 15:
		if (dev->out_iso_pipe == 0 || param->sglen == 0){
			printk("case 15 iso test %d %d\n",dev->out_iso_pipe , param->sglen);
			break;
		}
		dev_info(&intf->dev,
...
#if 0
		if (dev->int_pipe == 0 ){
			printk("null MMMM MMMM\n");
			break;}
		test_int(dev, param,dev->int_pipe,8, 0);
#endif
		retval = test_iso_queue(dev, param,
	...
	case 24:
		...
		break;
	case 25: /*add by wei.gou for int-in test */
		dev_info(&intf->dev, "TEST 25:  \n");
		retval = 0;
		for (i = param->iterations; retval == 0 && i > 0; --i) {
			}
		break;
...
module_param(force_interrupt, uint, S_IRUGO | S_IWUSR);
...
usbtest_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (force_interrupt || udev->speed == USB_SPEED_LOW) {
		...
		printk("%s %d \n",__FUNCTION__,__LINE__);
		if (info->ep_in) {
			dev->in_pipe = usb_rcvisocpipe(udev, info->ep_in);
			rtest = " iso-in";
			printk("%s %d \n",__FUNCTION__,__LINE__);
		}
		if (info->ep_out) {
			dev->out_pipe = usb_sndisocpipe(udev, info->ep_out);
			wtest = " iso-out";
			printk("%s %d \n",__FUNCTION__,__LINE__);
		}
		#endif
