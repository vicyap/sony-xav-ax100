...
void get_usb_device_port_num(struct us_data *us, struct Scsi_Host *host)
{
	struct usb_device	*p = us->pusb_dev;
	char p_num[10];
	int i,j;
	...
	host->sdu_name = kzalloc(5 + (i-1)*2, GFP_KERNEL);
	snprintf(host->sdu_name, 5 , "0sd%d", p->bus->busnum > 3 ?				\
										 p->bus->busnum - 3 : p->bus->busnum);

	for(j=0; j < i-1; j++){
		snprintf(host->sdu_name + j*2 + 4, 3, "-%d", p_num[i-j-2]);
	}

	//printk("usb port num: %s\n",host->sdu_name);
}
...
#ifdef CONFIG_USB_PORT_NAME
	get_usb_device_port_num(us, host);
#endif
