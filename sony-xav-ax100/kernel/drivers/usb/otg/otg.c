...
static struct usb_phy *phy_8388[2] = {NULL, NULL};
#endif
...
#ifdef CONFIG_USB_letb8388_OTG
struct usb_phy *usb_get_transceiver_8388(int bus_num)
{
	if (phy_8388[bus_num])
		get_device(phy_8388[bus_num]->dev);
	return phy_8388[bus_num];
}
EXPORT_SYMBOL(usb_get_transceiver_8388);
#endif
/**
...
#ifdef CONFIG_USB_letb8388_OTG
int usb_set_transceiver_8388(struct usb_phy *x, int bus_num)
{
	if (phy_8388[bus_num]&& x)
		return -EBUSY;
	phy_8388[bus_num]= x;
	return 0;
}
EXPORT_SYMBOL(usb_set_transceiver_8388);
#endif
