...
#include "f_rndis.c"
#include "rndis.c"
#define CONFIG_USB_ANDROID_SUNPLUS_COMPOSITE
...
#ifdef CONFIG_USB_ANDROID_SUNPLUS_COMPOSITE
//extern unsigned int nswitch;
#include "u_ncm.c"
#endif
#include "u_ether.c"
...
static int carplay_function_init(struct android_usb_function *f,
				 struct usb_composite_dev *cdev)
{
...
static void carplay_function_cleanup(struct android_usb_function *f)
{
	kfree(f->config);
	f->config = NULL;
	printk("%s %d \n", __FUNCTION__, __LINE__);
...
static int carplay_function_bind_config(struct android_usb_function *f,
					struct usb_configuration *c)
{
	int ret;
	int i;
...
static void carplay_function_unbind_config(struct android_usb_function *f,
					   struct usb_configuration *c)
{
	printk("%s %d \n", __FUNCTION__, __LINE__);
...
static struct android_usb_function audio_source_function = {
	.name = "audio_source",
#if 0
	.init = carplay_function_init,
	.cleanup = carplay_function_cleanup,
	.bind_config = carplay_function_bind_config,
	.unbind_config = carplay_function_unbind_config,
	//.attributes   = audio_source_function_attributes,
};
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	strncpy(manufacturer_string, "SunPlus",
		sizeof(manufacturer_string) - 1);
	strncpy(product_string, "letb8388", sizeof(product_string) - 1);
