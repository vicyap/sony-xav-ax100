...
#ifdef DEBUG_MSG
//#define udc_prnstr(x) prn_string(x)
//#define udc_prndword(x) prn_dword(x)
//#define udc_prnbyte(x) prn_byte(x)
#ifdef UBOOT
...
#ifdef UBOOT
#define USBD_BASE	0xf0070800//_USBD_BASE
#else
static __iomem USBD_BASE;
#endif

#define USBD_EXIT_FLAG	0xff
#define EXIT_TIME	(150*90)
...
static void letb123_udc_handle_ep0s_idle(unsigned char *buf)
{
	...
	udc_prnstr("bRequest  bRequestType . wValue  wIndex  wLength \n");
	udc_prndword(*(u32*)buf);
	udc_prndword(*(u32*)(buf+4));
...
void ep1_bulkin(u8 *buf,int len)
{
	...
	while((udc_read_(UDEP12FS) & 0x11) ||((!udc_read_(UDLIF))&EP1I_IF)){
		//udelay(100);
		printk("*%d&%x:%x",udc_read_(UDEP12FS),letb123_udc_get_ep_fifo_count(UDEP12PIC,1),letb123_udc_get_ep_fifo_count(UDEP12PIC,0));
