...
static int sd_probe(struct device *dev)
{
...
#ifdef CONFIG_USB_PORT_NAME

	if(sdp->host->sdu_name){

		int slen = 0;

		//while(slen <= DISK_NAME_LEN && sdp->host->sdu_name[slen++] != '\0');
		//slen = min_t(int, slen, DISK_NAME_LEN);

		slen = min_t(int, strlen(sdp->host->sdu_name) + 1, DISK_NAME_LEN - 1);

		if(sdp->host->sdu_name[0] != '0'){
			memcpy(gd->disk_name, sdp->host->sdu_name + 1, slen-2);
			snprintf(gd->disk_name + slen - 2, 3, "U%c", sdp->host->sdu_name[0]);
		}
		else
			memcpy(gd->disk_name, sdp->host->sdu_name+1, slen-1);

		sdp->host->sdu_name[0]++;
#if 1
		gd->disk_name[strlen(gd->disk_name)] = '-';
		gd->disk_name[strlen(gd->disk_name) + 1] = '\0';

		error = sd_format_disk_name(gd->disk_name, index, gd->disk_name, DISK_NAME_LEN);
		if (error) {
		...
		//gd->disk_name[strlen(gd->disk_name) + 1] = '\0';
#endif
		//error = sd_format_disk_name(sdp->host->sdu_name ? sdp->host->sdu_name : "sd", index, gd->disk_name, DISK_NAME_LEN);
	} else
	...
	//printk("sd-name: %s %s\n", gd->disk_name,sdp->host->sdu_name);
