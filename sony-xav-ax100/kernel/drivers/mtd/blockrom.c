			      ...
			      unsigned long block, char *buf)
{
	...
	ret = mtd_read(dev->mtd, addr, 512, &retlen, (uint8_t *) buf);

#if !defined(MTD_BLOCK_ROM_BIT_SCRUBBING)
        if(ret == -EUCLEAN) {
            // printk("%s, %d\n", __FILE__, __LINE__);
            ret = 0;    // Don't report this case (-EUCLEAN) to mtdblock layer.
        }
#endif
