		...
		struct uvc_buffer *buf, const __u8 *data, int len)
{
	...
	//if ((len >= 509 && len <= 512) || (len == 896))
	//	printk("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! UVC %d !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", len);

	/* Mark the buffer as bad if the error bit is set. */
	if (data[1] & UVC_STREAM_ERR) {
		printk("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! UVC: %p %d [%d,%d]: %2x %2x %2x %2x\n", data, len, stream->last_fid, fid, data[0], data[1], data[2], data[3]);
...
#if 1 // check UVC MJPG data error
		if (!buf->bytesused && urb->iso_frame_desc[i].actual_length > ret) {
			static char s[5] = "	";
			static u32 *fcc = (u32 *)s;
			if (*fcc != stream->cur_format->fcc) {
				*fcc = stream->cur_format->fcc;
				printk("!!!UVC FORMAT: %s\n", s);
			}
			if (*fcc == 0x47504A4D && *(u16*)(mem + ret) != 0xD8FF) { // MJPG
				printk("!!!UVC: ^ %02X ? ? ? ? ? ? ? ? ? ? ? %02X %02X %02X %02X %02X %02X %02X %02X\n", ret,
					mem[ret+0], mem[ret+1], mem[ret+2], mem[ret+3],
					mem[ret+4], mem[ret+5], mem[ret+6], mem[ret+7]);
			}
		}
#endif
...
static void uvc_video_complete(struct urb *urb)
{
	...
	if ((ret = usb_submit_urb(urb, GFP_ATOMIC)) < 0) {
		#if 0
		// TODO: 2014/11/06 retry (suggest by sj.huang)
		int retry = 10;

		while( retry -- ){
			ret = usb_submit_urb(urb, GFP_ATOMIC);
			if( !ret ){
				uvc_printk(KERN_ERR, "Retry %d times\n", 10 - retry + 1);
				break;
			}
		}

		if( ret ){ // retry 5 times but all failed!
			uvc_printk(KERN_ERR, "Failed to resubmit video URB (%d).\n",
				ret);
		}
		#else
		uvc_queue_cancel(&stream->queue, 1); //ADD by yong.y
		uvc_printk(KERN_ERR, "Failed to resubmit video URB (%d).\n",
			ret);
		#endif
	}
