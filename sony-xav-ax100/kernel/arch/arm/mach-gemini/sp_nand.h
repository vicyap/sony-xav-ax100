...
// #define CONFIG_SP_NAND_IOCTL // To report BCH ECC result
#ifdef CONFIG_SP_NAND_IOCTL
struct sp_nand_req {
	int page_addr;
	unsigned int ecc_sts;  // report BCH ecc status to user space, {31:16} Max_err_bit; {15:0}:allowed_err_bit
...
#ifdef CONFIG_SP_NAND_PING_PONG_BUFFER
	int buff_flag;  // buf-0 or buf-1 for upload data to HAL( MTD or FTL )
	int buff_page;
