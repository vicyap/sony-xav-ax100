...
#ifndef __MACH_IMGDEC_H__
#define __MACH_IMGDEC_H__

// IMGDEC IOCTL define:
#define IMGDEC_CMD_TYPE				'I'
#define IMGDEC_CMD_IMAGE_PARSE		_IOWR(IMGDEC_CMD_TYPE, 0x01, unsigned int)
#define IMGDEC_CMD_IMAGE_DECODE		_IOWR(IMGDEC_CMD_TYPE, 0x02, unsigned int)
#define IMGDEC_CMD_IMAGE_SCALE		_IOWR(IMGDEC_CMD_TYPE, 0x03, unsigned int)
/**************************************************************************************************
Notes for IMGDEC_CMD_IMAGE_PARSE:
(1) This driver only supports JPEG and PNG parser.

Notes for IMGDEC_CMD_IMAGE_DECODE:
(1) This driver only supports JPEG and PNG decoder.
(2) This driver only supports image (input) size in range as 65535 >= width/hieght >= 16 for JPEG, and 8192 >= width/hieght >= 16 for PNG.
(3) This driver only supports render (output) size in range as 8192 >= width/hieght >= 16 for JPEG and PNG.
(4) This driver supports default Huffman table for Motion-JPEG.
(5) This driver doesn't support interlaced PNG picture decoding.
(6) This driver doesn't support 16-bit (bit depth = 16) true color with alpha (color type = 6) PNG image format. 
(7) This driver only supports YUV420, RGB565, RGBA8888 and ARGB8888 output formats for JPEG and PNG pixel data.
(8) In parameters, all buffers with "phyaddr" word are allocated by chunk memory.

Notes for IMGDEC_CMD_IMAGE_SCALE:
...
// Data structure for IMGDEC_CMD_IMAGE_PARSE:
...
(8) image_color: after parsing, driver will write image encoding color type in this paramter.
                         ...
                         If image_type = 1 (PNG), the image encoding color type as follows:
                             0: GRAYSCALE
                             2: RGB
                             3: RGB_INDEX
                             4: GRAYSCALE_ALPHA
                             6: RGB_ALPHA
                             Note: image_color[0]=palette used; image_color[1]=color used; image_color[2]=alpha channel used;
(9) image_pixel: after parsing, driver will write image pixel type in this paramter.
                         0: Color
                         1: Gray
                         2: BW
(10) image_bkgd: after parsing PNG header, driver will write image background color in this paramter.
                         byte[3] = bKGD chunk number.
                         byte[2] = bKGD R value.
                         byte[1] = bKGD G value.
                         byte[0] = bKGD B value.
**************************************************************************************************/
...
// Data structure for IMGDEC_CMD_IMAGE_DECODE:
...
// Data structure for IMGDEC_CMD_IMAGE_SCALE:
