...
int sp_bch_init(struct mtd_info *mtd)
{
	...
	int i;
	int oobsz = mtd->oobsize;
	int pgsz = mtd->writesize;
	int rsvd = 32; /* Reserved bytes for YAFFS2 */
...
static long sp_bch_ioctl(struct file *filp, unsigned int cmd,
		unsigned long arg)
{
	struct sp_bch_chip *chip = &__this;
	struct sp_bch_regs *regs = chip->regs;
...
/* work with "modprobe sp_bch" from hotplugging or coldplugging */
MODULE_ALIAS("platform:sp_bch");
...
static int __init sp_bch_modinit(void)
{
//	if(gbootRom_boot_mode != 0x1D) return 0; /* main storage is not NAND, return! */

	return platform_driver_register(&sp_bch_driver);
}

static void __exit sp_bch_modexit(void)
{
...
module_init(sp_bch_modinit);
module_exit(sp_bch_modexit);
