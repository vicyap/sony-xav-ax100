...
#define REG_GROUP(g)		IO0_ADDRESS((g)*32*4)
#endif
#ifdef LOCAL_TEST
...
void DCACHE_CLEAN(u32 pa, void *start, int size)
{
	...
	/* clean L1 cache by range */
    dmac_map_area(start, size, DMA_TO_DEVICE);
	/* clean L2 cache by range */
	outer_clean_range(pa, pa + size);
...
{
	int ret;
	u32 len = simple_strtoul(buf, NULL, 10);
#ifdef TEST_CHUNKMEM
	void *p;
	if (len > 4096) {
		printk("len error %d\n", len);
	...
	p = gp_chunk_malloc_nocache(0, 0, len);
#else
...
static inline void rpc_add_seq(rpc_t *rpc)
{
	...
	if(!(rpc->SEQ_ADDR = (void *)gp_chunk_pa(addr))) {
...
{
	u32 seq = rpc->SEQ;
	u32 *addr = gp_chunk_va((u32)rpc->SEQ_ADDR);
	unsigned long timeout = jiffies + msecs_to_jiffies(10); // 10ms
	if(!addr) {
		addr = __va(rpc->SEQ_ADDR);
	}
	while(*addr != seq) {
		void *va = rpc->DATA_PTR;
		u32 pa =  gp_chunk_pa(va);
		if(!pa) {
			pa = __pa(va);
		}
...
static void rpc_copy(rpc_t *dst, rpc_t *src)
{
	...
	if (src == &IPC_REMOTE->RPC) {	// reg read IMPORTANT: DON'T read IPC_REMOTE twice!!!
		...
		if (len > RPC_DATA_SIZE) {
			...
			if (!(dst->DATA_PTR = gp_chunk_va(pa))) {
		...
		if (len > RPC_DATA_SIZE) {
			u32 pa;
			if (!(pa = gp_chunk_pa(src->DATA_PTR))) {
				pa = __pa(src->DATA_PTR);
			}
...
static void rpc_write_hw(rpc_t *rpc)
{
#ifndef IPC_USE_CBDMA
...
static int rpc_write(rpc_t __user *rpc_user)
{
	...
	if (rpc->F_DIR == RPC_REQUEST) {
		...
		rpc_write_hw(rpc);						// write response
		FREE(req);
	}
...
static int reg_server(int sid)
{
	...
	if (fifo) {
		...
		if (task && !task_is_stopped(task) && !task_is_dead(task)) {
			rcu_read_unlock();
 ...
 */
static int __init sp_ipc_init(void)
{
	int ret = -ENXIO;
	...
	if (ret != 0) {
		DIAG_ERROR("sp_ipc request_irq fail!\n");
		goto fail_reqirq;
	}
...
#endif
	device_create_file(ipc->dev.this_device, &dev_attr_test);
 ...
 */
static void __exit sp_ipc_exit(void)
{
	device_remove_file(ipc->dev.this_device, &dev_attr_test);
	misc_deregister(&ipc->dev);
	free_irq(IRQ_RPC, ipc);
	kthread_stop(ipc->rpc_res);
    mutex_destroy(&ipc->write_lock);
...
module_init(sp_ipc_init);
module_exit(sp_ipc_exit);

/**************************************************************************
 *                  M O D U L E    D E C L A R A T I O N                  *
 **************************************************************************/

MODULE_AUTHOR("Qin Jian");
