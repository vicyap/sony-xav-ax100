...
#ifdef CONFIG_SYS_MALLOC_LEN    /* U-Boot */
#include <malloc.h>
#endif
...
)
{
	...
	FATFS *fs;
	UINT i;
	BYTE possible_fat_without_mbr = 0;
	...
	*rfs = fs;							/* Return pointer to the file system object */
	...
	if(fmt == 0) {
		possible_fat_without_mbr = 1;
		fmt = 1;	// Pretend that there is a MBR, and the first partition is at sector 0. (when there is a MBR, check_fs(fs, 0) == 1)
	}
	...
	if (fs->fsize < (szbfat + (SS(fs) - 1)) / SS(fs))	/* (BPB_FATSz must not be less than the size needed) */
		return FR_NO_FILESYSTEM;
#else
	// Mantis: http://psweb.sunplus.com/mtdvd/view.php?id=269463
	// Bypass this check, becuase only read function is used.
	if (fs->fsize < (szbfat + (SS(fs) - 1)) / SS(fs)) {
		printf("\n\nWarning: Invalid BPB_FATSz\n  %s, %d: fs->fsize = 0x%x, szbfat = 0x%x, SS(fs) = 0x%x\n\n", __FILE__, __LINE__, fs->fsize, szbfat, SS(fs));
	}
#endif
		...
		if (res == FR_NO_FILE) {				/* Can create a new directory */
			...
			if (res == FR_OK) {					/* Initialize the new directory table */
...
#ifdef CONFIG_SYS_MALLOC_LEN    /* U-Boot */
				{
					...
					if ((dsc - dj.fs->fatbase) >= dj.fs->fsize ) { /* not in FAT area, refer to sync_window() for reason */
						mem_size = (dj.fs->csize) * SS(dj.fs);
						byte_ptr = malloc(mem_size);
						if(!byte_ptr) {
							printf("Error: %s, %d\n", __FILE__, __LINE__);  /* Please increase CONFIG_SYS_MALLOC_LEN */
							printf("mem_size = %d\n", mem_size);
							while(1);
						}
						memset(byte_ptr, 0, mem_size);
						memcpy(byte_ptr, dj.fs->win, sizeof(dj.fs->win));
						res = disk_write(dj.fs->drv, byte_ptr, dsc, dj.fs->csize);
						free(byte_ptr);
						// dsc += dj.fs->csize;
					} else {
...
)
{
	...
	for (i = 0; i < N_FATS; i++) {		/* Initialize each FAT copy */
...
#ifdef CONFIG_SYS_MALLOC_LEN    /* U-Boot */
		{
			...
			memset(byte_ptr, 0, mem_size);
			ret = disk_write(pdrv, byte_ptr, wsect, (n_fat - 1));
			free(byte_ptr);
...
#ifdef CONFIG_SYS_MALLOC_LEN    /* U-Boot */
	{
		...
		memset(byte_ptr, 0, mem_size);
		ret = disk_write(pdrv, byte_ptr, wsect, i);
		free(byte_ptr);
		wsect += i;
		if (ret != RES_OK)
			return FR_DISK_ERR;
