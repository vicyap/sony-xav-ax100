		     ...
		     size_t *retlen, uint8_t *buf)
{
	...
	*retlen = ops.retlen;
	...
	char isp_buf[16];
	sprintf(isp_buf, "0x%x", (u32)(from) + (u32)(*retlen));
	setenv("isp_addr_nand_read_next", isp_buf);
#endif
			     ...
			     int page)
{
    ...
    if(mtd->scan_oob_fast)
        chip->read_buf(mtd,chip->oob_poi,chip->badblock_pattern->len);
    else
#endif
	chip->read_buf(mtd, chip->oob_poi, mtd->oobsize);
	return 0;
			   ...
			   int cached, int raw)
{
	...
	if (!cached || !(chip->options & NAND_CACHEPRG)) {
		...
		if (status & NAND_STATUS_FAIL) {
			printf("%d page write fail,status:0x%x\n", page, status);
			return -EIO;
        }
	} else {
...
#if defined(CONFIG_SP_NAND)
	char isp_buf[16];
	sprintf(isp_buf, "0x%x", (u32)(to) + (u32)(*retlen));
	setenv("isp_addr_nand_write_next", isp_buf);
#endif
						  ...
						  const struct nand_flash_dev *type)
{
	...
	*dev_id = chip->read_byte(mtd);
	...
	printf("id=%02x%02x.%02x%02x%02x%02x ",
		id_data[0], id_data[1],
		id_data[2], id_data[3], id_data[4], id_data[5]);
#endif
