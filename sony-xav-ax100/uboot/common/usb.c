...
int usb_init(void)
{
	...
	for (i = 0; i < CONFIG_USB_MAX_CONTROLLER_COUNT; i++) {
		...
		dev = usb_alloc_new_device(ctrl, i);
		/*
	...
	do {
		/* init low_level USB */
		char *usb_dev;
		usb_dev = getenv("isp_dev");
		i = (int)(usb_dev[0] - '0');

		//printf("usb: isp dev %x %s\n", usb_dev[0], usb_dev);
		if(i >= CONFIG_USB_MAX_CONTROLLER_COUNT)
			i = 0;
 ...
 */
#ifdef CONFIG_USB_PORT_FASTEN
struct usb_device *usb_alloc_new_device(void *controller, int port_num)
{
	int i;
	debug("New Device %d\n", dev_index);
	...
	usb_dev[dev_index].port_num = port_num;
