...
static int usb_dev_port_map[CONFIG_USB_MAX_CONTROLLER_COUNT];
#endif
...
{
	if ((index >= CONFIG_USB_MAX_CONTROLLER_COUNT) || (usb_dev_port_map[index] >= usb_max_devs))
		return NULL;

	//printf("usb_dev_port_map %d %d %d\n", usb_dev_port_map[0], usb_dev_port_map[1], index);
	return &usb_dev_desc[usb_dev_port_map[index]];
}
...
#ifdef CONFIG_USB_PORT_FASTEN
			usb_dev_port_map[dev->port_num] = usb_max_devs;
#endif
