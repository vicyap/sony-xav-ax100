...
ecc_detected:
	printf("sp_bch: ecc mode=%ux%u, cr0=0x%x\n", size, bits, info->cr0);
	nand->ecc.size = size;
...
int sp_bch_decode(struct mtd_info *mtd, void *buf, void *ecc)
{
	...
	if (ret) {
		...
		ispsp_chk4badblock_err_bit_in_page = (uint32_t)(readl(&regs->esr));
		if(ispsp_chk4badblock_err_bit_in_page > ispsp_chk4badblock_max_err_bit_in_page)
			ispsp_chk4badblock_max_err_bit_in_page = ispsp_chk4badblock_err_bit_in_page;
	}
