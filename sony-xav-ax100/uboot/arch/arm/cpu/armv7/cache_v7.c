...
{
#if 0	// original u-boot code

	v7_dcache_maint_range(start, stop, ARMV7_DCACHE_INVAL_RANGE);

	v7_outer_cache_inval_range(start, stop);
#else
	// This modification is to align (start, stop) to line_len
	...
	if ((start & (line_len - 1)) || (stop & (line_len - 1))) {
		//stop &= ~(line_len - 1);                      // 8700
		stop = (stop + line_len - 1) & ~(line_len - 1); // new: complete covering for safty
		start &= ~(line_len - 1);
